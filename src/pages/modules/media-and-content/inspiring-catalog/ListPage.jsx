...
const initialRecords = Array.from({ length: 42 }).map((_, index) => ({
  id: index + 1,
  order: index + 1,
  mainMenu: `Catalog Menu ${index + 1}`,
  imagePreview: `https://picsum.photos/seed/catalog-${index + 1}/80/48`,
  isPublic: index % 2 === 0,
...
function Page({ onOpenDetail }) {
  ...
  const onRowMenuAction = (record, action) => {
    ...
    if (action === 'toggle-status') {
      setRecords((prev) =>
        prev.map((item) =>
          item.id === record.id ? { ...item, isPublic: !item.isPublic } : item,
        ),
      )
      return
    }
  ...
  return (
    <Stack gap="md">
      <Group justify="space-between" align="center">
        <div>
          <Title order={2}>Inspiring Catalog</Title>
          <Text c="dimmed">List view from specs/pages/Media & Content/Inspiring Catalog/table.md.</Text>
        </div>
